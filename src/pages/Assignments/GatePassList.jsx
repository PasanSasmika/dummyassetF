...
import {
  ...
  TbFileText as TbFileTextIcon, TbDeviceDesktop,
} from 'react-icons/tb';
...
const CATEGORY_ICONS = {
  ...
  'Tangible Information': TbFileTextIcon,
...
const openFile = (filePath) => {
  ...
  const url = filePath.startsWith('http') ? filePath : `${apiBase}/${cleanPath}`;
...
export default function GatePassList() {
  ...
  const allAssignments = Array.isArray(list) ? list : [];

  // Only assignments that have a gate pass uploaded
  const gatePasses = allAssignments.filter((a) => !!a.gate_pass_doc_id);

  // Stats
  const activeCount   = gatePasses.filter((a) => ['Assigned', 'Overdue'].includes(a.display_status || a.status)).length;
  const returnedCount = gatePasses.filter((a) => (a.display_status || a.status) === 'Returned').length;
  const withReturnNote = gatePasses.filter((a) => !!a.return_doc_id).length;

  // Filtering
  const filtered = gatePasses.filter((a) => {
    const status = a.display_status || a.status;
    ...
    const matchReturn =
      returnFilter === 'with'    ? !!a.return_doc_id  :
      returnFilter === 'without' ? !a.return_doc_id   : true;
    return matchText && matchStatus && matchReturn;
  });
  ...
  useEffect(() => { reset(); }, [appliedSearch, statusFilter, returnFilter]);

  const handleApply = () => setAppliedSearch(search);
  const handleClear = () => {
    setSearch(''); setAppliedSearch('');
    setStatusFilter(''); setReturnFilter('');
  };

  return (
    <MainLayout
      title="Gate Passes"
      subtitle="All assignments with gate pass documents and return notes"
    >
      {/* ── Stats ── */}
      <div className="grid grid-cols-4 gap-4 mb-6">
        <StatsCard label="Total Gate Passes" value={gatePasses.length}  icon="ticket"  color="green"   sub="All uploaded gate passes" />
        <StatsCard label="Active"             value={activeCount}        icon="chart"   color="blue"    sub="Currently assigned" />
        <StatsCard label="Returned"           value={returnedCount}      icon="active"  color="emerald" sub="Assets returned" />
        <StatsCard label="With Return Note"   value={withReturnNote}     icon="ticket"  color="amber"   sub="Return doc uploaded" />
      </div>
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <select
          ...
          className="px-3 py-2 rounded-lg border border-brand-cream bg-white text-sm
            focus:outline-none focus:ring-2 focus:ring-brand-green text-brand-dark/70"
        >
          <option value="">All Return Notes</option>
          <option value="with">With Return Note</option>
          <option value="without">Missing Return Note</option>
        </select>
        ...
        {(appliedSearch || statusFilter || returnFilter) && (
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-brand-cream/30 border-b border-brand-cream/60">
              {[
                'Asset', 'Assigned To', 'Gate Pass', 'Assignment Date', 'Status', 'Return Note', 'Actions',
              ].map((h) => (
                <th
                  key={h}
            ...
            {loading ? (
              <tr>
                <td colSpan={7} className="px-6 py-12 text-center">
                  ...
                  <p className="text-sm text-brand-dark/40">Loading gate passes...</p>
                </td>
              </tr>
            ) : paginated.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-6 py-16 text-center">
                  <div className="w-14 h-14 rounded-full bg-brand-cream/50 flex items-center justify-center mx-auto mb-3">
                    <TbDoor size={26} className="text-brand-dark/30" />
                  </div>
                  <p className="text-sm font-semibold text-brand-dark/50">No gate passes found</p>
                  <p className="text-xs text-brand-dark/30 mt-1">
                    {appliedSearch || statusFilter || returnFilter
                      ? 'Try adjusting your filters'
                      : 'Gate passes uploaded during assignment will appear here'}
                  </p>
                </td>
              </tr>
            ) : (
              paginated.map((a) => {
                const status    = a.display_status || a.status;
                const StatusIcon = STATUS_ICONS[status] || TbCircleDot;
                const CatIcon   = CATEGORY_ICONS[a.category] || HiOutlineCube;
                const hasReturnNote = !!a.return_doc_id;

                return (
                  <tr key={a.id} className={`transition-colors ${status === 'Overdue' ? 'bg-red-50/40 hover:bg-red-50' : 'hover:bg-brand-cream/20'}`}>

                    {/* Asset */}
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2.5">
                        <div className="w-9 h-9 rounded-xl bg-amber-100 flex items-center justify-center shrink-0">
                          <CatIcon size={16} className="text-amber-600" />
                        </div>
                        <div className="min-w-0">
                          ...
                          {a.category && (
                            <p className="text-[10px] text-brand-dark/30">{a.category}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      {a.assigned_to_name ? (
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-semibold text-brand-dark truncate max-w-[110px]">
                              {a.assigned_to_name}
                            </p>
                            {a.designation_title && (
                            ...
                            {a.assigned_by_name && (
                              <p className="text-[10px] text-brand-dark/30 truncate max-w-[110px]">
                    ...
                    <td className="px-4 py-4">
                      <div>
                        <div className="flex items-center gap-1.5 mb-1">
                          <FiFileText size={12} className="text-amber-500 shrink-0" />
                          <span className="text-xs font-semibold text-slate-700 truncate max-w-[140px]">
                            {a.gate_pass_doc_name || 'Gate Pass'}
                          </span>
                        </div>
                        {a.gate_pass_doc_uploaded_at && (
                          <p className="text-[10px] text-brand-dark/40 ml-4">
                            {fmt(a.gate_pass_doc_uploaded_at)}
                          </p>
                        )}
                        <button
                          onClick={() => openFile(a.gate_pass_doc_path)}
                          className="mt-1.5 ml-4 flex items-center gap-1 text-[11px] px-2.5 py-1
                            rounded-lg bg-amber-500 text-white font-semibold hover:bg-amber-600 transition"
                        >
                    ...
                    <td className="px-4 py-4">
                      ...
                      {a.condition_out && (
                        <span className={`mt-1 inline-block px-2 py-0.5 rounded-md text-[10px] font-bold ${CONDITION_COLORS[a.condition_out] || 'bg-slate-100 text-slate-500'}`}>
                          Out: {a.condition_out}
                        </span>
                      )}
                    </td>
                    ...
                    {/* Status */}
                    <td className="px-4 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${STATUS_COLORS[status] || 'bg-slate-100 text-slate-500'}`}>
                        <StatusIcon size={11} />
                        {status}
                      </span>
                      {status === 'Returned' && a.actual_return_date && (
                        <p className="text-[10px] text-brand-dark/40 mt-1 flex items-center gap-1">
                          <TbCircleCheck size={10} className="text-emerald-500" />
                          {fmt(a.actual_return_date)}
                        </p>
                      )}
                      {status === 'Returned' && a.condition_in && (
                        <span className={`mt-1 inline-block px-2 py-0.5 rounded-md text-[10px] font-bold ${CONDITION_COLORS[a.condition_in] || 'bg-slate-100 text-slate-500'}`}>
                          In: {a.condition_in}
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-4">
                      {hasReturnNote ? (
                        <div>
                          ...
                          <button
                            onClick={() => openFile(a.return_doc_path)}
                            className="mt-1.5 ml-4 flex items-center gap-1 text-[11px] px-2.5 py-1
                        ...
                        <div className="flex items-center gap-1.5 text-xs text-brand-dark/30">
                          <TbFileOff size={14} />
                          {status === 'Returned' ? 'Not uploaded' : 'Not returned yet'}
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center rounded-lg
                          bg-brand-cream/50 hover:bg-brand-cream text-brand-dark/50 transition-colors"
      ...
      {totalItems > 0 && (
        <p className="text-xs text-brand-dark/40 mt-3 px-1">
          Showing {startIndex}–{endIndex} of {totalItems} gate pass records
        </p>
      )}
    </MainLayout>
  );
